...
def main():
    ...
    parser.add_argument('--task_data', default=[], nargs='+', help='task data files to download. Any of: ' + ','.join(list(TASK_FILES.keys())))
    ...
    if args.type:
        if not set(args.type) & set(FILETYPES):
            print(('ERROR: Invalid file type: ' + ', '.join(args.type)))
            return
