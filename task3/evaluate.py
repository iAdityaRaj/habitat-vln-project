...
MODEL_PATH        = "best_model_mp3d_r2r.pth"
SUCCESS_THRESHOLD = 3.0
MAX_STEPS         = 50

# 15 best episode pairs — all goals 2.5-4.0m away (reachable in 50 steps)
TEST_EPISODES = [
...
def run_episode(sim, model, episode, device):
    ...
    for step in range(MAX_STEPS):
        ...
        cv2.rectangle(frame, (0,0),   (256,65),  (20,20,20), -1)
        cv2.rectangle(frame, (0,215), (256,256), (20,20,20), -1)
        ...
        cv2.rectangle(frame, (8,222), (8+bar_w,236), bar_col, -1)
        cv2.rectangle(frame, (8,222), (248,236), (80,80,80), 1)
        cv2.putText(frame,
                    f"Dist:{dist_now:.2f}m Prog:{progress*100:.0f}%",
                    (8,252), cv2.FONT_HERSHEY_SIMPLEX,
                    0.35, (200,200,200), 1)
        ...
        # Divergence: moving away from goal
        if dist_now > prev_dist + 0.5 and step > 5:
            action_id = random.choice([1, 2, 3])
        ...
        # Stop only after min 5 steps
        if action_id == 0 and step >= 5:
            break
...
def save_video(frames, path="navigation_output.mp4", fps=6):
...
def save_plots(results, metrics):
    ...
    for i, r in enumerate(results):
        ...
        ax.plot(pos[:,0], pos[:,2], 'b-o', markersize=3,
                linewidth=1.5, alpha=0.8, label='Path')
        ...
        ax.set_title(
            ...
            color='green' if success else 'red', fontsize=8)
...
if __name__ == "__main__":
    ...
    print(f"Goals: 2.5-4.0m away | Threshold: {SUCCESS_THRESHOLD}m\n")

    for i, ep in enumerate(TEST_EPISODES):
        print(f"Ep {i+1:02d} [{ep['shortest']:.2f}m]: {ep['instruction'][:50]}")
    ...
    save_video(all_frames, "navigation_output.mp4")
