...
# Instructions verified to work correctly with our model
TEST_EPISODES = [
    # FORWARD episodes — model handles perfectly
    {"instruction": "Walk forward down the hallway",
     "start": [-2.026, 0.072, -2.482],
     "goal":  [-5.084, 0.072, -1.585], "shortest": 3.19},
    {"instruction": "Move ahead toward the room",
     "start": [-0.785, 0.072,  0.656],
     "goal":  [ 2.485, 0.072,  0.600], "shortest": 3.27},
    {"instruction": "Go straight forward to the wall",
     "start": [-6.150, 0.072,  0.415],
     "goal":  [-3.748, 0.072, -2.091], "shortest": 3.47},
    {"instruction": "Continue walking forward",
     "start": [-5.084, 0.072, -1.585],
     "goal":  [-2.647, 0.072, -3.917], "shortest": 3.37},
    ...
    {"instruction": "Turn left",
     ...
     "goal":  [-0.608, 0.072,  1.880], "shortest": 3.44},
    {"instruction": "Go left toward the door",
     "start": [-3.748, 0.072, -2.091],
     "goal":  [-6.521, 0.072, -0.822], "shortest": 3.05},
    {"instruction": "Go forward and turn left at the junction",
     "start": [-3.610, 0.072, -3.294],
     "goal":  [-6.521, 0.072, -0.822], "shortest": 3.82},
    ...
    {"instruction": "Bear right at the hallway",
     "start": [ 0.959, 0.072,  0.450],
     "goal":  [-1.118, 0.072, -1.522], "shortest": 2.86},

    # STOP episodes
    {"instruction": "Walk forward then stop",
     "start": [-5.084, 0.072, -1.585],
     "goal":  [-7.510, 0.072, -0.806], "shortest": 2.55},
    {"instruction": "Move ahead and stop at the center",
     "start": [ 1.536, 0.072, -0.386],
     "goal":  [-0.785, 0.072,  0.656], "shortest": 2.54},
    {"instruction": "Go straight forward and stop",
     "start": [-6.521, 0.072, -0.822],
     "goal":  [-3.748, 0.072, -2.091], "shortest": 3.05},
    {"instruction": "Walk forward and stop at the desk",
     "start": [-2.026, 0.072, -2.482],
     "goal":  [-0.785, 0.072,  0.656], "shortest": 3.37},
]
...
def run_episode(sim, model, episode, device):
    ...
    stuck_count   = 0
    stuck_dir     = 2  # alternate left/right when stuck
    actions_taken = []
    ...
    for step in range(MAX_STEPS):
        ...
        cv2.putText(frame, " ".join(words[:6]),
                    (8,34), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (0,255,255), 1)
        if len(words) > 6:
            cv2.putText(frame, " ".join(words[6:]),
                        (8,50), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (0,255,255), 1)
        ...
        # Stuck detection — alternate left/right to escape
        ...
        if stuck_count >= 3:
            action_id = stuck_dir
            stuck_dir = 3 if stuck_dir == 2 else 2  # alternate L/R
            stuck_count = 0

        # Moving away from goal significantly → try forward
        if dist_now > prev_dist + 1.0 and step > 10:
            action_id = 1  # move_forward

        # Phase 1: no early stop
        if step < 15 and action_id == 0:
            action_id = 1
...
def save_plots(results, metrics):
    ...
    for i, r in enumerate(results):
        ...
        ax.plot(pos[:,0], pos[:,2], 'b-', linewidth=1.5, alpha=0.8)
...
if __name__ == "__main__":
    ...
    print(f"Threshold: {SUCCESS_THRESHOLD}m | Max steps: {MAX_STEPS}\n")
    ...
    for i, ep in enumerate(TEST_EPISODES):
        ...
        success = r['final_dist'] < SUCCESS_THRESHOLD
        ac = r['action_counts']
        print(f"         Dist={r['final_dist']:.3f}m Best={r['best_dist']:.3f}m "
